...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Found product', productData});
  } catch (err) {
    res.status(500).json(err);
  }
});
...
router.post('/', async (req, res) => {
  ...
  */
  ...
  Product.create(req.body)
    ...
    .then((productTagIds) => res.status(200).json({ message: 'Successfully created product and paired to tags', productTagIds}))
    .catch((err) => {
...
router.put('/:id', async (req, res) => {
  ...
  Product.update(req.body, {
    ...
    .then((updatedProductTags) => res.json({ message: 'Successfully updated product tags', updatedProductTags}))
    .catch((err) => {
      res.status(400).json(err);
    });
});
